...
**2.** With the rapid expansion of generative AI, Transformer models have become widely adopted, but their internal Attention mechanism poses a bottleneck due to its quadratic complexity in both computation and memory.
   - Source: *Power Efficient Attention Acceleration on CGLA*
   - Domain: `transformer_efficiency`

**3.** FlashAttention has emerged as an efficient algorithm that reduces memory access and enables faster computation while maintaining exact numerical output.
   - Source: *Power Efficient Attention Acceleration on CGLA*
   - Domain: `transformer_efficiency`

**4.** In this work, we propose a power-efficient realization of FlashAttention on IMAX3, a linear array-based Coarse-Grained Reconfigurable Array (CGRA) that we refer to as Coarse-Grained Linear Array (CGLA) accelerator.
   - Source: *Power Efficient Attention Acceleration on CGLA*
   - Domain: `transformer_efficiency`

**5.** Transformers are slow and memory-hungry on long sequences, since the time and memory complexity of self-attention are quadratic in sequence length.
